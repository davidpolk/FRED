...
#define PRESCALER_ARRAY_TIMER2_SIZE     (6)
...
class L6474 {
  ...
  // void L6474_setup(L6474 myL6474)
    void AttachFlagInterrupt(void (*callback)(void));     //Attach a user callback to the flag Interrupt
    void Begin(uint8_t nbShields, int nShield);                        //Start the L6474 library
    ...
    int32_t GetMark(uint8_t shieldId, int nShield);                    //Return the mark position 
    ...
    int32_t GetPosition(uint8_t shieldId, int nShield);                //Return the ABS_POSITION (32b signed)
    void GoHome(uint8_t shieldId, int nShield);                        //Move to the home position
    void GoMark(uint8_t shieldId, int nShield);                        //Move to the Mark position
    void GoTo(uint8_t shieldId, int32_t targetPosition, int nShield);  //Go to the specified position
    void HardStop(uint8_t shieldId, int nShield);                      //Stop the motor and disable the power bridge
    void Move(uint8_t shieldId,                           //Move the motor of the specified number of steps
              dir_t direction,
              uint32_t stepCount, int nShield);    
    void ResetAllShields(int nShield = 0);                              //Reset all L6474 shields
    void Run(uint8_t shieldId, dir_t direction, int nShield);             //Run the motor 
    ...
    void SetHome(uint8_t shieldId, int nShield);                          //Set current position to be the home position
    void SetMark(uint8_t shieldId, int nShield);                          //Set current position to be the Markposition
    ...
    ///@{
    void CmdDisable(uint8_t shieldId, int nShield);              //Send the L6474_DISABLE command
    void CmdEnable(uint8_t shieldId, int nShield);               //Send the L6474_ENABLE command
    uint32_t CmdGetParam(uint8_t shieldId,          //Send the L6474_GET_PARAM command
                                 L6474_Registers_t param, int nShield);
    uint16_t CmdGetStatus(uint8_t shieldId, int nShield);        // Send the L6474_GET_STATUS command
    void CmdNop(uint8_t shieldId, int nShield);                  //Send the L6474_NOP command
    void CmdSetParam(uint8_t shieldId,              //Send the L6474_SET_PARAM command
                             L6474_Registers_t param,       
                             uint32_t value, int nShield);
    uint16_t ReadStatusRegister(uint8_t shieldId, int nShield);  // Read the L6474_STATUS register without
                                                    // clearing the flags
    ...
    void SelectStepMode(uint8_t shieldId,           // Step mode selection
                                L6474_STEP_SEL_t stepMod, int nShield);     
    ...
    void StepClockHandler(uint8_t shieldId, int nShield); 
    ///@}
  ...
  private:
    ...
    void SendCommand(uint8_t shieldId, uint8_t param, int nShield);
    void SetRegisterToPredefinedValues(uint8_t shieldId, int nShield);
    void WriteBytes(uint8_t *pByteToTransmit, uint8_t *pReceivedByte, int nShield);    
    ...
    void SetShieldParamsToPredefinedValues(int nShield);
    void StartMovement(uint8_t shieldId, int nShield);
